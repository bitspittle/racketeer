...
class BuyCardCommand(ctx: GameContext, private val card: Card) : Command(ctx) {
    private val shouldMarkNew get() = ctx.settings.admin.highlightNewCards && !ctx.cardStats.contains(card.template.name)
    ...
    override suspend fun invoke(): Boolean {
        ctx.runStateChangingAction {
            ...
            ctx.state.apply(GameStateChange.MoveCard(card, ctx.state.discard))
        }
        return true
    }

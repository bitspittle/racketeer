...
class ViewBlueprintCommand(ctx: GameContext, blueprint: Blueprint, shouldMask: Boolean = blueprint.shouldMask(ctx)) : Command(ctx) {
    ...
    override val description = if (shouldMask) "You must build this building at least once to see its details." else ctx.describer.describeBlueprintBody(blueprint, includeFlavor = true, showBuildCost = true)
}

...
class CardListCommand(ctx: GameContext) : Command(ctx) {
    ...
    override val description: String = "Look through all the cards in the game and see what you've used (${ctx.data.cards.count { ctx.userStats.cards.contains(it.name) } * 100 / ctx.data.cards.size}%)."
